    ...
    <div className="page">
      <div className="page-body pad" style={{ paddingTop: 'calc(env(safe-area-inset-top,0px) + 26px)', paddingBottom: 26, display: 'flex', flexDirection: 'column' }}>
        ...
        <p style={{ color: 'var(--muted)', margin: '10px 0 22px', fontSize: 15, lineHeight: 1.5 }}>
          {STOPS.length} stops. {STOPS.length} riddles. A hidden duck at every one. This is not a race, so take the whole day.
        </p>
                ...
                {DUCKS.map((d) => (
                  <button key={d.id} onClick={() => setDuck(d.id)}
                    style={{
                      ...
                      background: duckId === d.id ? 'rgba(201,162,39,.18)' : 'rgba(236,234,226,.05)',
                      border: `1px solid ${duckId === d.id ? 'rgba(201,162,39,.55)' : 'var(--line)'}`
                    }}>
                    <Duck body={d.body} bill={d.bill} size={26} />
                  </button>
                ))}
              </div>
          ...
          {i === 2 && (
            <motion.div key="c" {...step} transition={{ duration: 0.26 }}>
                ...
                {RIG_COLORS.map((c) => (
                  <button key={c.id} onClick={() => setRig(c.id)} title={c.name}
                    style={{
                      ...
                      border: `2px solid ${rigId === c.id ? 'var(--brass)' : 'rgba(0,0,0,.35)'}`,
                      boxShadow: rigId === c.id ? '0 0 0 3px rgba(201,162,39,.22)' : 'none'
              ...
              <div className="card" style={{ padding: 18, margin: '20px 0 18px', display: 'flex', gap: 14, alignItems: 'center' }}>
                <div className="crew-duck" style={{ width: 38, height: 38 }}>
                  <Duck {...DUCKS.find((d) => d.id === duckId)} size={22} />
                </div>
                <div>
